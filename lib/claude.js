...
export async function generateMeeting({ theme, participants, length, difficulty, previous }) {
...
- ${noise}`;
  ...
  if (previous) {
    user += `\n\n【重要】この会議は、以下の前回会議の「続編」です。数週間後に開かれた設定で、次を自然に盛り込んでください:
- 前回の決定事項の進捗報告(順調なもの・遅れているもの)
- 前回保留・積み残しになった論点の続きの議論
- 前回の決定が引き起こした新たな問題や想定外の展開
- 参加者は基本的に前回と同じ顔ぶれ(1名の追加・欠席は可)
# 前回の会議
タイトル: ${previous.title}
ログ:
${previous.transcript.slice(0, 6000)}`;
  }
